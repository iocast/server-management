...
# Source:
#   based on the script from Marcp
...
###########################################
#### TEMPORARLY EXPORT PGPASSFILE PATH ####
###########################################

if [ $ENABLE_PGPASS_FILE = "yes" ]
then
	export PGPASSFILE=$SCRIPTPATH/pgpass.conf
fi
...
function perform_backups()
{
	...
	do
		SCHEMA_ONLY_CLAUSE="$SCHEMA_ONLY_CLAUSE datname ~ '$SCHEMA_ONLY_DB' or"
	done

	SCHEMA_ONLY_QUERY="select datname from pg_database where ${SCHEMA_ONLY_CLAUSE:1:-3} order by datname;"
